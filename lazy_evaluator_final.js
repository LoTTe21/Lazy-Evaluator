...
function apply_primitive_function(fun, argument_list) {
    display("Apply_primitive_function");
    display(fun);
...
function apply(fun, args, env) {
    display("Calling apply function");
   display(fun);
   display(args);
   if (is_primitive_function(fun)) {
       display("is_primitive_function");
      return apply_primitive_function(
          fun, 
          list_of_arg_values(args, env));
          //change the args to a list of value
...
function lookup_name_value(name, env) {
    function env_loop(env) {
        ...
        if (is_empty_environment(env)) {
            error(name, "Unbound name: the empty environment");
            // the env is empty so no name can be found
        } else {
            const frame = first_frame(env);
            // why we only use the first frame?
...
function is_pair_statement(stmt)
{
    return head(tail(operator(stmt))) === "pair";
}

function eval_pair_statement(stmt, env)
{
    display("Calling eval_pair_statement");
    const first = delay_it(head(stmt), env);
    const rest = delay_it(head(tail(stmt)), env);
    display(pair(first, rest));
...
parse_and_eval("-12 - 8;");
*/
//parse_and_eval("const a = 1;");
//head(tail(head(setup_environment())));
//parse_and_eval("function factorial(n) { return n === 1 ? 1 : n;} factorial(4);");
...
(parse_and_eval("const a = pair(1, 2); head(a);"));
...
["application", [["name", ["head", null]], [[["name", ["a", null]], null], null]]]
